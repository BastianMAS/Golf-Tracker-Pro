...
function saveQualityTests(qualityKey) {
    ...
    quality.tests.forEach(test => {
        if (test.bilateral) {
            const leftValue = parseFloat(document.getElementById(test.inputs[0])?.value);
            const rightValue = parseFloat(document.getElementById(test.inputs[1])?.value);
            
            if (!isNaN(leftValue) || !isNaN(rightValue)) {
                testData.tests[test.key] = {
                    left: !isNaN(leftValue) ? leftValue : null,
                    right: !isNaN(rightValue) ? rightValue : null
                };
                hasData = true;
            }
        } else {
            const value = parseFloat(document.getElementById(test.input)?.value);
            if (!isNaN(value)) {
